...
symmetric_graph<symmetric_vertex, int> read_symmetric_graph(
    const char* fname, bool mmap, bool binary, char* bytes, size_t bytes_size) {
  size_t n, m;
  uintT* offsets;
  uintE* edges;
  ...
  auto e_weights = gbbs::new_array_no_init<std::tuple<uintE, int>>(m);
  parallel_for(0, m, [&](size_t i) {e_weights[i]=std::make_tuple(edges[i],1);});
  return symmetric_graph<symmetric_vertex, int>(
      v_data, n, m,
      [=]() {
        gbbs::free_array(v_data, n);
        if (!binary) gbbs::free_array(edges, m);
